...
public class FloorPlacedObject : PlacedObject
{
    ...
    public void PlaceEdge(Edge edge, BuildingSystemPartSO floorEdgeObjectTypeSO)
    {
    ...
    public void Load(string saveString)
    {
        ...
        if (floorSaveObject.upEdgeObjectName != "")
            PlaceEdge(Edge.Up, HouseBuildingSystem.instance.GetPlacedObjectTypeSOFromName(floorSaveObject.upEdgeObjectName));
        if (floorSaveObject.downEdgeObjectName != "")
            PlaceEdge(Edge.Down, HouseBuildingSystem.instance.GetPlacedObjectTypeSOFromName(floorSaveObject.downEdgeObjectName));
        if (floorSaveObject.leftEdgeObjectName != "")
            PlaceEdge(Edge.Left, HouseBuildingSystem.instance.GetPlacedObjectTypeSOFromName(floorSaveObject.leftEdgeObjectName));
        if (floorSaveObject.rightEdgeObjectName != "")
            PlaceEdge(Edge.Right, HouseBuildingSystem.instance.GetPlacedObjectTypeSOFromName(floorSaveObject.rightEdgeObjectName));
    }
}